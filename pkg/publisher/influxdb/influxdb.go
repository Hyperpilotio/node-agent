...
func getConfig(config snap.Config) (configuration, error) {
	cfg := configuration{}
	var err error
...
func (ip *InfluxPublisher) Publish(metrics []snap.Metric, pluginConfig snap.Config) error {
...
func replaceDynamicElement(m snap.Metric) ([]string, map[string]string) {
	tags := map[string]string{}
...
func groupCommonNamespaces(m snap.Metric, tags map[string]string, mpoints map[string]point) {
