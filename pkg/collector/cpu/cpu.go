...
func (p *CPUCollector) GetMetricTypes(cfg snap.Config) ([]snap.Metric, error) {
	...
	for _, namespace := range namespaces {
		...
		mt := snap.Metric{
			Namespace: snap.NewNamespace(strings.Split(namespace, string(os.PathSeparator))...)}
		ns := mt.Namespace
		...
		if !mList[mItem.Value] {
			...
			mts = append(mts, snap.Metric{
				Namespace: snap.NewNamespace(strings.Split(prefix, string(os.PathSeparator))...).
			...
			for cpuId, cpuStats := range p.stats {
				for k, v := range cpuStats {
					if ns[len(ns)-1].Value == k && v != nil {
						ns1 := make([]snap.NamespaceElement, len(ns))
						copy(ns1, ns)
...
func (p *CPUCollector) init(cfg snap.Config) error {
