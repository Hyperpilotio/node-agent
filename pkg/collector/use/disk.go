...
func getDiskMetricTypes() ([]snap.Metric, error) {
	var mts []snap.Metric

	for _, diskName := range listDisks() {
		for _, name := range metricLabels {
			mts = append(mts, snap.Metric{
				Namespace: snap.NewNamespace("intel", "use", "storage", diskName, name)})
		}
...
func (u *Use) diskStat(ns snap.Namespace) (*snap.Metric, error) {
